...
class Config 
{
   ...
   * @return Config
   */
  public function __construct( $config_name, $process = false, $full_path="")
  {
    $this->_config_name = $config_name;
	if(file_exists(TO_ROOT . "/core/config/templates/{$config_name}.ini"))
	{
		$this->_filename = TO_ROOT . "/core/config/templates/{$config_name}.ini";
	} else if (file_exists(TO_ROOT . "/config/templates/{$config_name}.ini")) {
		$this->_filename = TO_ROOT . "/config/templates/{$config_name}.ini";
	}
